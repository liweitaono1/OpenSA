...
from audit.models import JobsResults
...
class CrontadbList(LoginPermissionRequired, ListView):
    ...
    def get_queryset(self):
        self.queryset = JobsResults.objects.filter(type=4, project__name=self.get_project()).exclude(
            task_schedule__id=None)
...
class ExecutionList(LoginPermissionRequired, ListView):
    ...
    def get_queryset(self):
        self.queryset = JobsResults.objects.filter(type=0, project__name=self.get_project()).exclude(
            task_schedule__id=None)
        date_from = self.request.GET.get('date_from')
        date_to = self.request.GET.get('date_to')
        operator = UserProfile.objects.get(email=self.request.user)
...
class ExecutionResult(LoginPermissionRequired, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        kwargs.update(context)
        return super(ExecutionResult, self).get_context_data(**kwargs)
...
class ScriptsList(LoginPermissionRequired, ListView):
    ...
    def get_queryset(self):
        self.queryset = JobsResults.objects.filter(type=1, project__name=self.get_project()).exclude(
            task_schedule__id=None)
        date_from = self.request.GET.get('date_to')
        date_to = self.request.GET.get('date_to')
        operator = UserProfile.objects.get(email=self.request.user)
        if self.request.GET.get('name') and operator.is_superuser:
            query = self.request.GET.get('name', None)
            ...
            if date_from and date_to:
                ...
                self.queryset = self.queryset.filter(start_time__gte=date_from).filter(
                    start_time__lte=end).order_by(
                    '-start_time')
            else:
                self.queryset = self.queryset.order_by('-start_time')
...
class SchedulingList(LoginPermissionRequired, ListView):
    ...
    def get_queryset(self):
        self.queryset = JobsResults.objects.filter(type=3, project__name=self.get_project()).exclude(
            task_schedule__id=None)
        date_from = self.request.GET.get('date_from')
        date_to = self.request.GET.get('date_to')
        operator = UserProfile.objects.get(email=self.request.user)
...
class BatchFilesList(LoginPermissionRequired, ListView):
    ...
    def get_queryset(self):
        self.queryset = JobsResults.objects.filter(type=2, project__name=self.get_project()).exclude(
            task_schedule__id=None)
        date_from = self.request.GET.get('date_from')
        date_to = self.request.GET.get('date_to')
        operator = UserProfile.objects.get(email=self.request.user)
